...
workflow  {
	...
	// previous modified_bin_ch = BINNING.out.bin_ch.map { file -> tuple(file.simpleName.unique().toString().replaceAll('_bin', '')[1..-2], file) }
	...
	// previous comment joined_unitig_bam_ch = UNITIG_PER_BIN.out.unitig_ch.cross(joined_strainy_bam_ch)
	...
	// previous comment join_read_and_vcf_ch = reads_ch.join(UNZIP_SNP.out.unzip_snp_ch)
	// previous comment join_bam_bai_gfa_ch = SUBSET_BAM.out.bam_ch.join(INDEX_BAM.out.bam_bai_ch.join(SUBSET_GFA.out.subset_gfa_ch))
	 
	// previous comment all_join_ch = join_read_and_vcf_ch.join(join_bam_bai_gfa_ch)
	...
	// BIN_TRANSFORM(STRAINY.out.strainy_final_ch)
       ...
       // previous comment bam_output_dir_ch = channel.fromPath( final_params.output_dir )
	// bams = "bams"
	// bam_output_dir_ch = channel.fromPath("${final_params.output_dir}/${bams}")
	
			  
	// previous comment quality_report_bam_ch = MAG_QA.out.quality_report_ch.combine(bam_output_dir_ch)
	
	// FILTER_BY_MAG_QUALITY(MAG_QA.out.quality_report_ch, bam_output_dir_ch)
	
	// previous comment quality_report_bam_to_tuple_ch = FILTER_BY_MAG_QUALITY.out.quality_bam_ch.map { file -> tuple(file.simpleName.toString()[0..-1], file.baseName, file }
	...
	// previous comment join_coverage_and_transformed_fa_ch = COUNT_MAG_COVERAGE.out.count_mag_ch.combine(transformed_fa_ch)
	...
	// previous comment MAG_TRANSFORMED_QA(FILTER_BY_COVERAGE.out.fa_mag_ch.collect(  sort: {a, b -> a[0].getBaseName() <=> b[0].getBaseName()} ))
